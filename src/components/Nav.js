...
function Nav() {
    ...
    const TOKEN_REFRESH_INTERVAL = 15000; // Refresh token every 15 seconds (before 20s expiration)
    ...
    //         const response = await axios.post('http://localhost:8080/identity/auth/refresh', {
    //             token: currentToken
    //         }, {
    //             headers: {
    ...
    const handleLogout = async () => {
        try {
            ...
            if (token) {
                ...
                await axios.post('http://localhost:8080/identity/auth/logout', {
                    token: token
                }, {
                    headers: {
                        'Content-Type': 'application/json'
