...
const GroupDetailPage = () => {
            ...
            try {
                ...
                const isDuplicate = postComments[postId]?.some(
                    comment => comment.detail === commentText && 
                               comment.username === userInfo.email && 
                ...
                if (userAvatarUrl && optimisticComment.commentId) {
                    setCommentAvatarUrls(prev => ({
                        ...prev,
                        [optimisticComment.commentId]: userAvatarUrl
                    }));
                }
                                                ...
                                                <>
                                                    ...
                                                    <div className='group-comment-divided'>
                                                        ...
                                                        {(showComments[post.id] && commentPagination[post.id]?.totalElements > 0) && (
                                                            <div className="group-comments-list">
                                                                {commentLoading[post.id] && (
                                                                    <div className="group-comments-loading">
                                                                        <div className="group-comments-loading-spinner"></div>
                                                                        <p>Đang tải nhận xét...</p>
                                                                    </div>
                                                                )}
                                                                
                                                                {!commentLoading[post.id] && postComments[post.id]?.length === 0 && (
                                                                    <div className="group-no-comments">
                                                                                                        ...
                                                                                                        <div key={reply.commentId} className="group-reply-item">
                                                                                                            <div className="group-comment-avatar-container">
                                                                                                                {commentAvatarUrls[reply.commentId] ? (
                                                                                                                    ...
                                                                                                                    />
                                                                                                                ) : reply.username === userInfo.email && userAvatarUrl ? (
                                                                                                                    // Nếu là reply của người dùng hiện tại
                                                                                                                    <img 
                                                                                                                        src={userAvatarUrl} 
                                                                                                                        alt="Avatar" 
                                                                                                                        className="group-comment-avatar-small"
                                                        ...
                                                        <div className="group-comment-section">
                                                            {userAvatarUrl ? (
                                                                <img src={userAvatarUrl} alt="Avatar" className='comment-avatar'/>
                                                            ) : (
        ...
        if (e.key === 'Enter' && commentInput[postId]?.trim()) {
            try {
                ...
                const replyMessage = {
                    ...
                    replyUsername: userInfo.email, // Current user's username
                ...
                // Store user's avatar URL for this temporary reply if available
                if (userAvatarUrl && optimisticReply.commentId) {
                    setCommentAvatarUrls(prev => ({
                        ...prev,
                        [optimisticReply.commentId]: userAvatarUrl
                    }));
                }
    ...
    const handleIncomingReply = (replyData) => {
        ...
        console.log('Is reply from current user?', formattedReply.username === userInfo?.email);
        ...
        if (formattedReply.avatar) {
            ...
            // Nếu không có avatar từ server, kiểm tra xem reply có phải của học sinh hiện tại không
            if (formattedReply.username === userInfo.email) {
                // Nếu là reply của học sinh hiện tại, sử dụng avatar hiện tại
                if (userAvatarUrl) {
                    console.log('Using user avatar for reply:', formattedReply.commentId);
                    setCommentAvatarUrls(prev => ({
                        ...prev,
                        [formattedReply.commentId]: userAvatarUrl
                    }));
                }
            }
        }
    ...
    const fetchCommentAvatar = async (avatarPath, commenterId, username) => {
        ...
        try {
            ...
            // Kiểm tra xem có phải avatar của người dùng hiện tại không
            if (userInfo && userInfo.avatar === avatarPath && userInfo.email === username) {
                console.log(`Using cached user avatar for ${commenterId} because username matches: ${username}`);
                if (userAvatarUrl) {
                    setCommentAvatarUrls(prev => ({
                        ...prev,
                        [commenterId]: userAvatarUrl
                    }));
                    return;
                }
            } else if (userInfo && userInfo.avatar === avatarPath) {
                console.log(`Avatar path matches current user but username doesn't match. Current: ${userInfo.email}, Reply: ${username}`);
            }
            ...
            // Nếu lỗi và là avatar của người dùng hiện tại, sử dụng avatar hiện tại
            if (userInfo && userInfo.avatar === avatarPath && userInfo.email === username && userAvatarUrl) {
                setCommentAvatarUrls(prev => ({
                    ...prev,
                    [commenterId]: userAvatarUrl
                }));
    ...
    // Hàm gọi API để lấy ra ảnh đại diện của người dùng hiện tại
    const fetchUserAvatar = async (avatarPath) => {
        if (!avatarPath) return;
    ...
    const fetchUserInfo = async () => {
        try {
            ...
            if (response.data && response.data.code === 0) {
                ...
                // Fetch avatar if available
                if (userData.avatar) {
                    fetchUserAvatar(userData.avatar);
                }
            }
        } catch (error) {
